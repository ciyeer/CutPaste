...
float CCommonHelper::widthMultiplyingPower_ = 0;
float CCommonHelper::heightMultiplyingPower_ = 0;
...
void CCommonHelper::setLanguagePack(const QString &language) {
    // 加载中文包
...
#include <QApplication>
void CCommonHelper::moveCenter(QWidget *widget, QRect parentRect) {
    if (parentRect.isEmpty()) {
        //parentRect = QGuiApplication::primaryScreen()->geometry()->rect();
...
bool CCommonHelper::getSmallestWindowFromCursor(QRect& out_rect) {
    HWND hwnd;
    POINT pt;
    // 获得当前鼠标位置
    ::GetCursorPos(&pt);
...
bool CCommonHelper::getCurrentWindowFromCursor(QRect &out_rect)
{
    HWND hwnd;
    POINT pt;
    // 获得当前鼠标位置
