...
CCAmplifier::CCAmplifier(std::shared_ptr<QPixmap> originPainting,QWidget *parent)
    : QWidget(parent),
    ...
    /// 设置默认大小
    sideLength_ = 122 * CCommonHelper::getWindowHeightMultiplyingPower();
    imageHeight_ = 90 * CCommonHelper::getWindowHeightMultiplyingPower();
...
void CCAmplifier::onSizeChange(int w, int h) {
    screenSize_ = QSize(w, h);
}

void CCAmplifier::onPostionChange(int x, int y) {
...
void CCAmplifier::paintEvent(QPaintEvent *) {
