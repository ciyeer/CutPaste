...
CCScreenshot * CCScreenshot::self_ = nullptr;
bool CCScreenshot::isActivity_ = false;
bool CCScreen::isInit_ = false;

CCScreenshot::CCScreenshot(QWidget *parent) : QWidget(parent),
 ...
 * @return：CCScreenshot
 */
CCScreenshot *CCScreenshot::Instance() {
    ...
    if (!self_) {
       ...
       if (!self_) {
           isActivity_ = true;
           self_ = new CCScreenshot;
       }
    }
    return self_;
}

void CCScreenshot::destroy() {
    if (!isActivity_ && self_) {
        delete self_;
        self_ = nullptr;
    }
...
void CCScreenshot::hideEvent(QHideEvent *) {
    isActivity_ = false;
}


void CCScreenshot::closeEvent(QCloseEvent *) {
    isActivity_ = false;
}

void CCScreenshot::mouseDoubleClickEvent(QMouseEvent *) {
    emit doubleClick();
}
 ...
 * 初始化放大镜 (色彩采集器)
 */
void CCScreenshot::initAmplifier(std::shared_ptr<QPixmap> originPainting) {
    std::shared_ptr<QPixmap>  temp_pm = originPainting;
    if (temp_pm == nullptr) {
        temp_pm = originPainting_;
    }
    amplifierTool_.reset(new CCAmplifier(temp_pm, this));
...
void CCScreenshot::initMeasureWidget()
{
    rectTool_.reset(new CCRect(this));
    rectTool_->raise();
}
 ...
 * 功能：获得当前屏幕的大小
 */
const QRect &CCScreenshot::getScreenRect() {
...
std::shared_ptr<QPixmap> CCScreenshot::initGlobalScreen() {
 ...
 * 返回：QPixmap* 指针
 */
std::shared_ptr<QPixmap> CCScreenshot::getGlobalScreen() {
 ...
 * 参数：_ico 鼠标图片的资源文件
 */
void CCScreenshot::initCursor(const QString& ico) {
    QPixmap pixmap;
    if (ico.isEmpty()) {
...
std::shared_ptr<CCScreen> CCScreenshot::createScreen(const QPoint &pos) {
    if (screenTool_.get() == nullptr) {
        /// 创建截图器
        screenTool_.reset(new CCScreen(originPainting_, pos, this));
        /// 建立信号连接
        connect (this, SIGNAL(cursorPosChange(int,int)),
...
void CCScreenshot::destroyScreen() {
    if (screenTool_.get() != nullptr) {
        /// 断开信号资源
...
void CCScreenshot::mousePressEvent(QMouseEvent *e) {
...
void CCScreenshot::mouseReleaseEvent(QMouseEvent *e) {
...
void CCScreenshot::mouseMoveEvent(QMouseEvent *e) {
...
void CCScreenshot::paintEvent(QPaintEvent *) {
...
void CCScreenshot::updateMouse(void) {
    /// 获取当前鼠标选中的窗口
    ::EnableWindow((HWND)winId(), FALSE);
    /// @marker: 只更新一次,可以修复用户误操作导致的查找窗口与识别界面窗口不一致.
    CCommonHelper::getSmallestWindowFromCursor(windowRect_);
...
void CCScreenshot::keyPressEvent(QKeyEvent *e) {
    /// Esc 键退出截图;
...
CCRect::CCRect(QWidget *parent) : QWidget(parent) {
...
void CCRect::paintEvent(QPaintEvent *) {
    QPainter painter(this);
...
void CCRect::onPostionChange(int x, int y) {
    if (x < 0) x = 0;
    if (y < 0) y = 0;
...
void CCRect::onSizeChange(int w, int h) {
...
CCScreen::CCScreen(std::shared_ptr<QPixmap> originPainting, QPoint pos, QWidget *parent)
...
CCScreen::DIRECTION CCScreen::getRegion(const QPoint &cursor) {
    if (!isInit_) {
        return NONE;
    }
    CCScreen::DIRECTION ret_dir = NONE;
    // left upper
...
void CCScreen::contextMenuEvent(QContextMenuEvent *) {
    /// 在鼠标位置弹射出菜单栏
    menu_->exec(cursor().pos());
}

void CCScreen::mouseDoubleClickEvent(QMouseEvent *e) {
    if (e->button() == Qt::LeftButton) {
        emit doubleClick();
        e->accept();
    }
...
void CCScreen::mousePressEvent(QMouseEvent *e) {
    if (e->button() == Qt::LeftButton) {
        isPressed_ = true;
        if(direction_ != NONE) {
            this->mouseGrabber();
...
void CCScreen::mouseReleaseEvent(QMouseEvent * e) {
    if (e->button() == Qt::LeftButton) {
        isPressed_ = false;
        if(direction_ != NONE) {
...
void CCScreen::mouseMoveEvent(QMouseEvent * e) {
...
#ifndef QT_NO_DEBUG
            qDebug() << "CCScreen::" << __FUNCTION__ << "call onMouseChange()";
#endif
            /// 鼠标进行拖拉拽
            switch(direction_) {
            case LEFT:
...
void CCScreen::moveEvent(QMoveEvent *) {
    emit postionChange(x(), y());
}

void CCScreen::resizeEvent(QResizeEvent *) {
    listMarker_.clear();
...
void CCScreen::showEvent(QShowEvent *) {
    isInit_ = true;
}

void CCScreen::hideEvent(QHideEvent *) {
    currentRect_ = {};
    movePos_ = {};
    originPoint_ = {};
    isInit_ = false;
...
void CCScreen::enterEvent(QEvent *e) {
    setCursor(Qt::SizeAllCursor);
    QWidget::enterEvent(e);
}

void CCScreen::leaveEvent(QEvent *e) {
    setCursor(Qt::ArrowCursor);
    QWidget::leaveEvent(e);
}

void CCScreen::closeEvent(QCloseEvent *)
{
    isInit_ = false;
}

void CCScreen::paintEvent(QPaintEvent *) {
    QPainter painter(this);
    /// 绘制截屏编辑窗口
...
void CCScreen::quitScreenshot() {
    close();
    parentWidget()->close();
}

void CCScreen::onMouseChange(int x, int y) {
    show();
    if (x < 0 || y < 0) {
        return;
    }
...
extern "C" void CCScreenshot() {
    CCScreenshot::Instance();
}
