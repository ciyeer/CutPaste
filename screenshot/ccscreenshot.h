...
#ifndef CCSCREENSHOT_H
#define CCSCREENSHOT_H
 ...
 * @brief : 截屏功能的主要入口,
 *          管理全局热键,资源的回收与释放.
 * @remark: 调用示例( CCScreenshot::Instance(); )
*/
class CCScreenshot : public QWidget {
    Q_OBJECT
...
public:
     ...
     * @date  :2021年09月3日
     */
    explicit CCScreenshot(QWidget *parent = 0);
    ~CCScreenshot();

    /**
     * @brief : 窗口实例
     * @note  : 通过这个函数获得截图器的整个实例
     * @return: 返回 CCScreenshot 实例指针
     * @date  : 2021年09月3日
     */
    static CCScreenshot *Instance();
...
private:
     ...
     * @date  : 2021年09月3日
     */
    std::shared_ptr<QPixmap> initGlobalScreen();
     ...
     */
    std::shared_ptr<CCScreen> createScreen(const QPoint &pos);
     ...
     */
    std::shared_ptr<QPixmap> getGlobalScreen();
...
private:
    ...
    /// 截图屏幕
    std::shared_ptr<CCScreen>   screenTool_;
    /// 截图器大小感知器
    std::shared_ptr<CCRect>     rectTool_;
    /// 放大取色器
    std::shared_ptr<CCAmplifier> amplifierTool_;
    /// 当前鼠标选区最小的矩形窗口
    QRect                       windowRect_;
    /// 截屏实例对象
    static CCScreenshot         *self_;
    /// 置顶定时器
...
public:
    ...
    explicit CCRect(QWidget *parent = nullptr);
...
public:
    ...
    explicit CCScreen(std::shared_ptr<QPixmap> originPainting, QPoint pos, QWidget *parent = 0);

    ~CCScreen() { isInit_ = false; }
